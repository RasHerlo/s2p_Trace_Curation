...
class AnalysisToolsWindow(QDialog):
    def __init__(self, main: Any) -> None:
        ...
        self.lbl_method = QLabel(
            "Placeholder only: Run uses pickle order of currently selected "
            "(iscell=True) traces. PCA, hierarchical clustering, and similarity "
    ...
    def _set_draft_form(self) -> None:
        ...
        idx = self.cmb_kind.findData(KIND_PLACEHOLDER)
        if idx >= 0:
    ...
    def _on_run(self) -> None:
        ...
        self.lbl_preview.setText(
            f"Preview: {len(order)} selected ROI(s) — {kind_label(kind)}. Save to keep."
        )
    ...
    def _on_rebuild(self) -> None:
        ...
        try:
            ...
            return
        apply_run_result(run, doc, roi_ids=roi_ids, order=order)
        self._preview = {
            "kind": kind,
            "params": params,
            "roi_ids": roi_ids,
